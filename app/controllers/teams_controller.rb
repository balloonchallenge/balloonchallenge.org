class TeamsController < ApplicationController
  ...
  def create
    ...
    if @team.save
      ...
      @team.update_attributes(:signed_up_2020 => true)
  ...
  def admin
    ...
    # 2020 categories (same as 2017)

    @education = AttachedAsset.where(category: 'Best Educational Initiative').where(["created_at > ?", Time.new(2020, 1, 1, 0, 0, 0)]).order('team_id ASC')
    @design    = AttachedAsset.where(category: 'Best Design').where(["created_at > ?", Time.new(2020, 1, 1, 0, 0, 0)]).order('team_id ASC')
  ...
  def launch
    @teams = Team.where(signed_up_2020: true).where.not(launch_date: '').order('launch_date ASC')
  end
  private
